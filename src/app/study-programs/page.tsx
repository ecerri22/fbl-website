...
const allPrograms = [
  {
    ...
    programs: [
      { name: "Administrim-Biznes", level: "Bachelor" },
      { name: "Administrim-Biznes", level: "Master profesional" },
      { name: "Menaxhim", level: "Master i shkencave" },
    ],
  },
  {
    department: "Departamenti i Financë-Kontabilitetit",
    programs: [
      { name: "Financë-Kontabilitet", level: "Bachelor" },
      { name: "Financë", level: "Master profesional" },
      { name: "Drejtim bankar", level: "Master i shkencave" },
      { name: "Kontabilitet", level: "Master profesional" },
      { name: "Kontabilitet dhe Auditim", level: "Master i shkencave" },
    ],
  },
  {
    department: "Departamenti i së Drejtës",
    programs: [
      { name: "Ekonomia dhe e drejta", level: "Bachelor" },
      { name: "Ekonomia dhe e drejta ne sektorin publik", level: "Master profesional" },
      { name: "Ekonomia dhe e drejta në tregjet ndërkombëtare", level: "Master i shkencave" },
      { name: "Drejtësi", level: "Diplomë e Integruar Master i shkencave" },
    ],
  },
  {
    department: "Departamenti i Ekonomiksit",
    programs: [
      { name: "Informatike ekonomike", level: "Bachelor" },
      { name: "Informatike ekonomike", level: "Master profesional" },
    ],
  },
  {
    department: "Departamenti i Marketing Inxhinierisë",
    programs: [
      { name: "Ekonomi turizmi", level: "Bachelor" },
      { name: "Administrim biznesi dhe Inxhinieri", level: "Bachelor" },
      { name: "Marketing", level: "Master profesional" },
      { name: "Marketing", level: "Master i shkencave" },
    ],
  },
];
...
export default function StudyProgramsPage() {
  ...
  useEffect(() => {
    const flatPrograms = allPrograms.flatMap(dept =>
    ...
    if (typeFilter === 'bachelor') {
      setFilteredPrograms(flatPrograms.filter(p =>
      ...
      setFilteredPrograms(flatPrograms)
    }
  }, [typeFilter])
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10">
          {filteredPrograms.map((program, i) => (
            ...
            >
              ...
              <div className="mt-6">
                <button className="text-sm text-red-700 hover:underline font-medium">Learn More →</button>
              </div>
            </div>
          ))}
        </div>
